...
# ---------------- DIARY SETUP ----------------
diary_bg = pygame.image.load("bg.jpg")
diary_bg = pygame.transform.scale(diary_bg, (WIDTH, HEIGHT))

diary_book = pygame.image.load("jr.png")
diary_book = pygame.transform.scale(diary_book, (700, 450))

diary_title_font = pygame.font.SysFont("brush script", 42, True)
diary_text_font = pygame.font.SysFont("brush script", 30)
...
def reset_game():
    ...
    global score, journal_msg, journal_timer
    canvas.fill((0, 0, 0, 0)) 
...
while running:
    ...
    if game_state == "PLAYING":
    ...
    elif game_state == "DIARY":
        screen.blit(diary_bg, (0, 0))
        screen.blit(diary_book, (book_x, book_y))

        title = diary_title_font.render("My Diary", True, COL_WHITE)
        screen.blit(title, (WIDTH//2 - title.get_width()//2, 35))

        lines = wrap_text(diary_text)

        # LEFT PAGE
        y = start_y
        for i in range(min(len(lines), max_lines_left)):
            txt = diary_text_font.render(lines[i], True, (0,0,0))
        ...
        # OPEN DIARY
        if event.key == pygame.K_d and game_state == "PLAYING":
            game_state = "DIARY"

        if game_state == "DIARY":

            if event.key == pygame.K_BACKSPACE:
                diary_text = diary_text[:-1]
